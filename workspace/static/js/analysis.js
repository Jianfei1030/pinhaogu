...
(function() {
  ...
  const metaText = document.getElementById('analysisMetaText');
  ...
  toggleBtn.addEventListener('click', () => {
    ...
    if (!panel.hidden) {
      // 面板打开时检查是否有正在运行的分析
      checkAndRestoreRunningAnalysis();
    }
  });
  ...
  historySelect.addEventListener('change', async () => {
    ...
    try {
      const res = await fetch(`/api/premarket/report?date=${historySelect.value}`);
      if (res.ok) {
        const data = await res.json();
        resultEl.innerHTML = renderPremarketReport(data);
      }
    } catch(e) {}
  });

  // 检查并恢复正在运行的分析状态
  async function checkAndRestoreRunningAnalysis() {
    if (isRestoring) return;
    isRestoring = true;

    try {
      const res = await fetch(`/api/premarket/status?date=${encodeURIComponent(dateInput.value)}`);
  ...
  function restoreProgressUI(status) {
    ...
    resultEl.innerHTML = '<div style="color:var(--muted); text-align:center; padding:20px;">盘前分析中，请稍候...</div>';
  }
  ...
  runBtn.addEventListener('click', async () => {
    ...
    try {
      const startRes = await fetch('/api/premarket/run', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ date: dateInput.value || undefined }) });
      ...
      resultEl.innerHTML = `<div style="color:#ef4444;padding:12px;background:rgba(239,68,68,0.1);border-radius:6px;">❌ 盘前分析启动失败<br><span style="font-size:13px;">${String(e.message || '未知错误')}</span></div>`;
      runBtn.disabled = false;
      runBtn.textContent = '▶ 运行盘前分析';
    }
  });
  ...
  async function pollStatus() {
    try {
      ...
      const statusMap = { loading:'正在加载数据...', analyzing:'盘前分析中...', sending:'发送 Telegram...', saving:'保存报告...', done:'✅ 盘前分析完成', error:'❌ 出错' };
      ...
      if (s.status === 'done') {
        ...
        runBtn.disabled = false;
        runBtn.textContent = '▶ 运行盘前分析';
        const rRes = await fetch(`/api/premarket/report?date=${encodeURIComponent(dateInput.value)}`);
        if (rRes.ok) {
  ...
  // 渲染盘前分析报告
  function renderPremarketReport(data) {
    if (!data || !data.date) {
      return '<div style="color:var(--muted); text-align:center; padding:20px;">暂无盘前分析报告</div>';
    }
    
    const rec = data.recommended_sector || {};
    ...
    let html = `
      <div style="padding:12px; background:var(--bg-card); border-radius:8px; margin-bottom:12px;">
        ...
        <div style="margin-bottom:8px;">
          <span style="font-size:18px; font-weight:700; color:var(--text);">${rec.name || '-'}</span>
          <span style="font-size:13px; color:var(--muted); margin-left:6px;">${rec.code || ''}</span>
        </div>
        <div style="font-size:22px; font-weight:700; color:${colorPct(rec.change_pct)}; margin-bottom:8px;">
  ...
  function renderStructuredAnalysis(llmText) {
    ...
    html += '<div style="font-size:13px; font-weight:600; color:var(--text); margin-bottom:12px; padding-bottom:10px; border-bottom:1px solid var(--border);">🤖 AI 盘前分析</div>';
