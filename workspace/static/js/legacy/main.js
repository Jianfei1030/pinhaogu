...
function initTabSwitcher() {
  ...
  tabBar.addEventListener('click', (e) => {
    ...
    // 切到新闻 Tab 时，强制重载当天全量新闻，避免沿用增量游标
    if (tabName === 'news' && typeof window.reloadNewsFullDay === 'function') {
      window.reloadNewsFullDay();
    }
...
async function init() {
  ...
  loadRecentNews(true);
  setInterval(() => loadRecentNews(false), 60000);
